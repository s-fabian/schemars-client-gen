...
impl RequestInfo {
    pub(crate) fn append_function(
        ...
        writeln!(buffer, "{TAB}{name}.PATH = '{}';\n", self.path)?;

        self.append_error_codes(&name, buffer)?;
        self.append_deprecation(&name, buffer)?;

        if self.res_body.is_websocket() {
            write!(
                buffer,
                "    export function {name}({req_params}): {struct_name}Websocket {{
        const wsBaseUrl = new URL(options.baseUrl, typeof location !== 'undefined' ? \
                 location.href : undefined);

        return new WebsocketWrapper(
            () => new WebSocket(
                `${{wsBaseUrl}}${{{name}.PATH}}{params_suffix}`
            ),
        ...
        return new SSE(
            () => new EventSauce(
                new Request(
                    options.baseUrl + {name}.PATH{params_suffix},
                    {{
                        method: '{method}',
                        body: {req},
                        credentials: 'include',
            ...
            (data) => options.unsafe ? data as {struct_name}Msg : {name}Msg.parse(data),
                    ...
                    headers_addition = if self.req_body.is_schema() {
                        "\nheaders: jsonContentTypeHeader(init.headers as \
                         RepresentsHeader, options.globalInit.headers as \
                         RepresentsHeader),"
                    } else {
                        ""
                    },
            ...
            options.fetch(
                new Request(
                    ...
                    res = match &self.res_body {
                        Kind::None =>
                            ".then(res => res.ok ? ok(res) : err(res))".to_string(),
                        Kind::Any => ".then(res => res.ok ? res.text().then(ok) : \
                                      err(res))"
                            .to_string(),
                        Kind::Schema(_) => format!(
                            ".then(res => res.ok ? res.json().then(options.unsafe ? \
